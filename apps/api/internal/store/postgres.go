...
func (p *Postgres) ListRuns() []RunRecord {
	...
	// ⚡ Bolt: Fixed N+1 query problem. Fetching all runs and their associated reports in a single query.
	// Expected improvement: Reduces database queries from O(N) to O(1) for list runs, significantly decreasing latency when many runs exist.
	rows, err := p.db.QueryContext(ctx, `
		SELECT
			runs.id::text,
